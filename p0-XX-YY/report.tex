...
\textbf{4-a) } The min and max values of img-green respectively are 0 and 255.	The mean value is X and the deviation is X. The computation for all these values were made using the python library numpy, the computation for the min and max value is pretty straightforward they are the highest and lowest values in the image, the computation of the mean value is explained in Figure \ref{fig:mean}, where \textbf{X} is each pixel in the image and \textbf{n} is the total amount of pixels in the image, the standard deviation value is explained in Figure \ref{fig:deviation}, where \textbf{X} is every pixel in the image, \textbf{X|} is the mean value and \textbf{N} is the total amount of pixels in the image.
...
\begin{figure}[!h]
	...
	{%
		\setlength{\fboxsep}{1pt}%
		\setlength{\fboxrule}{1pt}%
		\fbox{\includegraphics[scale=0.8]{mean}}%
	}%
...
\begin{figure}[!h]
	...
	{%
		\setlength{\fboxsep}{1pt}%
		\setlength{\fboxrule}{1pt}%
		\fbox{\includegraphics[scale=0.8]{deviation}}%
	}%
...
After the previous steps we subtracted the shifted image to the original colored one, making sure all the values were valid ones, Figure \ref{fig:p0-4-c-1} is the output.
